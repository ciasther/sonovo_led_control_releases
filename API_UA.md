...
- Обмежувач частоти починає з burst у 40 запитів і відновлює 20 запитів за секунду. Після вичерпання burst сервер повертає 429. Ліміт спільний для всіх клієнтів процесу.
...
Поле color відображає форму введення. Після PUT з назвою це рядок; після PUT з #RRGGBB або об'єктом це об'єкт із r, g і b. Обробляйте обидві форми:
...
- 405 - Метод не підтримується. Використовуй GET або PUT відповідно до опису ендпоінта. Для шляхів під /api/v1/ токен перевіряється першим, тому запит без коректного токена отримує 401, а не 405.
...
- 429 - Ліміт запитів вичерпано. Зменш частоту. Початковий burst дорівнює 40, далі доступно 20 запитів за секунду. Ліміт спільний для всіх клієнтів.
...
- 401 unauthorized - заголовок Authorization містить застарілий або неправильний токен, або процес не може прочитати файл токена. Вміст файлу обрізається під час запуску, тому пробільні символи навколо токена у файлі не шкодять. У Linux файл має права 0600.
